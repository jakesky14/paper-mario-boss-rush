...
import { createRings, rotateRing, slideColumn, simulatePath, applyHolePunchSpecial, applyOrigamiKingSpecial, ensureBacksideReachable, NUM_PANELS, shuffleArray, } from './rings';
...
export class Game {
    ...
    placeBumperBands() {
        ...
        for (let r = 0; r < 4; r++) {
            for (let s = 0; s < 12; s++) {
                ...
                if (p === 'arrow_up' || p === 'arrow_down') {
                    positions.push({ ring: r, slot: s, origPanel: p });
                }
            }
        }
        ...
        for (const pos of toReplace) {
            rings[pos.ring].panels[pos.slot] = 'rubber_band';
            newBands.push(pos);
        }
    ...
    handleKey(e) {
        ...
        if (this.state.paused)
            return; // block all other keys while paused
        // Testing select
        ...
        if (phase === 'main_menu') {
            if (e.key === 'ArrowUp') {
                ...
                if (this.state.mainMenuCursor === 'fight') {
                    ...
                    try {
                        localStorage.removeItem('pmBossRush');
                    }
                    catch (_) { /* ignore */ }
        ...
        if (phase !== 'puzzle')
            return;
        if (this.state.rotationAnim !== null)
            return;
    ...
    startEnemyTurn() {
        if (this.state.bossIndex === 1 && !this.state.rubberBandSoloMode) {
            const nextPhase = Math.random() < 0.5 ? 'snapback' : 'rubber_bind';
    ...
    applyFinalDamage(type, perfect) {
        const heartBonus = this.state.maxUpHeartsBought * 2;
        const quality = this.state.attackQuality;
        // Base damage per quality tier
        const jumpDmg = { none: 6, nice: 7, great: 8, excellent: 9 };
        const hammerDmg = { none: 7, nice: 8, great: 10, excellent: 12 };
    ...
    computeMaxHp() {
        ...
        if (this.state.accessories.heartPlus)
            base += 5;
        if (this.state.accessories.silverHeartPlus)
            base += 10;
        if (this.state.accessories.goldHeartPlus)
            base += 20;
        return base;
    }
    computeMaxTimer() {
    ...
    rollHeartBonus() {
        let bonus = 0;
        if (this.state.accessories.heartPlus)
            bonus += Math.random() < 0.5 ? 10 : 15;
        if (this.state.accessories.silverHeartPlus)
            bonus += Math.random() < 0.5 ? 20 : 25;
        if (this.state.accessories.goldHeartPlus)
            bonus += Math.random() < 0.5 ? 30 : 35;
        return bonus;
    }
    tryBuyItem() {
        const SHOP_ITEMS = [
        ...
        if (item.key === 'maxUpHeart') {
            if (this.state.maxUpHeartsInStock <= 0)
                return;
            if (100 + this.state.maxUpHeartsBought * 20 >= 200)
                return; // cap reached
            ...
            if (this.state.accessories[accKey])
    ...
    doSaveAndExit() {
        ...
        try {
            localStorage.setItem('pmBossRush', JSON.stringify(saveData));
        }
        catch (_) { /* ignore */ }
        this.returnToMenu();
    }
    ...
    hasSaveData() {
        try {
            return !!localStorage.getItem('pmBossRush');
        }
        catch (_) {
            return false;
        }
    ...
    startGame() {
        ...
        if (save) {
            ...
            if (s.accessories && typeof s.accessories === 'object')
                this.state.accessories = { ...this.state.accessories, ...s.accessories };
            if (typeof s.maxUpHeartsBought === 'number')
                this.state.maxUpHeartsBought = s.maxUpHeartsBought;
            if (typeof s.maxUpHeartsInStock === 'number')
                this.state.maxUpHeartsInStock = s.maxUpHeartsInStock;
            if (Array.isArray(s.bossFirstClear))
                this.state.bossFirstClear = s.bossFirstClear;
        }
        const baseMax = this.computeMaxHp();
        const heartBonus = this.rollHeartBonus();
        const maxHp = baseMax + heartBonus;
        const startBoss = save && typeof save.nextBossIndex === 'number'
            ? Math.min(save.nextBossIndex, 5)
            : 0;
    ...
    computeMaxHpFrom(acc, maxUpBought) {
        let base = 100 + maxUpBought * 20;
        if (acc.heartPlus)
            base += 5;
        if (acc.silverHeartPlus)
            base += 10;
        if (acc.goldHeartPlus)
            base += 20;
        return base;
    }
    generateConfetti() {
    ...
    update(dt) {
        ...
        if (this.state.paused)
            return; // freeze all updates while paused
        const state = this.state;
        ...
        if (state.envelopeTimer > 0)
            state.envelopeTimer = Math.max(0, state.envelopeTimer - dt);
        switch (state.phase) {
            case 'title':
            ...
            case 'whole_punch_arms': {
                if (!state.wholePunchMashDone) {
                    ...
                    if (state.wholePunchMashCooldown > 0)
                        state.wholePunchMashCooldown = Math.max(0, state.wholePunchMashCooldown - dt);
            ...
            case 'trapped_snapback': {
                ...
                if (state.trappedSnapbackTimer <= 0) {
                    const snapRanges = [[20, 25], [16, 21], [14, 19], [12, 17]];
            ...
            case 'mario_walk': {
                ...
                if (state.marioWalkTimer >= stepDuration) {
                    ...
                    if (state.bossIndex === 0) {
                        const isTargeted = state.targetedPanels.some(tp => tp.ring === step.ring && tp.slot === step.slot);
                        if (isTargeted) {
                            const targetDmg = this.applyGuardReduction(3);
                            state.playerHp = Math.max(0, state.playerHp - targetDmg);
                            state.targetedPanels = state.targetedPanels.filter(tp => !(tp.ring === step.ring && tp.slot === step.slot));
                            state.damageNumbers.push({
                ...
                if (state.attackProjectileT >= IMPACT_T) {
                    ...
                    if (state.playerHp <= 0) {
                        ...
                        if (this.bossSpecialPending) {
                            if (state.boss.special === 'tape')
                                applyHolePunchSpecial(state.rings);
                            if (state.boss.special === 'origami_king' || state.boss.special === 'scissors')
                                applyOrigamiKingSpecial(state.rings);
                            this.bossSpecialPending = false;
                        }
                        this.transitionTo('setup');
                    }
            ...
            case 'solo_snapback_attack': {
                ...
                if (state.soloSnapbackAttackT >= 1) {
                    state.blockWindowOpen = false;
                    const snapRanges = [[56, 61], [53, 58], [50, 55], [45, 50]];
                    const [minDmg, maxDmg] = snapRanges[Math.min(3, state.marioFinalRing)];
