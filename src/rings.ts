...
export function createRings(dist: PanelDistribution): RingState[] {
  ...
  const limitedPanels: PanelType[] = ['treasure_chest', 'on_panel', 'envelope'];
...
export function ensureActionReachable(rings: RingState[], startSlot = 6): void {
  ...
  const keep: PanelType[] = ['magic_circle', 'on_panel', 'plus_one', 'double_power'];
  for (let c = 0; c < NUM_PANELS; c++) {
