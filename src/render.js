...
function drawJumpAnimation(ctx, state) {
    ...
    if (!state.attackTimingPressed) {
        ...
        const qualityText = { excellent: 'EXCELLENT!', great: 'GREAT!', nice: 'NICE!', none: 'HIT!' };
        const txt = qualityText[state.attackQuality];
        const qualityColor = { excellent: '#ffdd00', great: '#88ff44', nice: '#44ddff', none: '#aaaaaa' };
...
function drawHammerGauge(ctx, state, tick) {
    ...
    if (!state.attackTimingPressed) {
        ...
        ctx.font = 'bold 22px monospace';
        ctx.textAlign = 'center';
        const qualityColor2 = { excellent: '#ffdd00', great: '#88ff44', nice: '#44ddff', none: '#aaaaaa' };
        const qualityLabel = { excellent: 'EXCELLENT!', great: 'GREAT!', nice: 'NICE!', none: 'HIT!' };
...
function drawRubberBandProjectile(ctx, x, y, spin) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(spin);
    ctx.beginPath();
...
function drawArmsGrab(ctx, state, tick) {
    ctx.save();
    ctx.fillStyle = '#000a14';
    ctx.fillRect(0, 0, CANVAS_W, CANVAS_H);
...
function drawPauseButton(ctx, state, tick) {
...
function drawEnvelopeOverlay(ctx, state) {
    if (!state.envelopeMessage || state.envelopeTimer <= 0)
        return;
...
function drawSnapbackPhase(ctx, state, tick) {
...
function drawRubberBindPhase(ctx, state, tick) {
...
function drawTrappedSnapback(ctx, state, tick) {
    const TOTAL_MS = 3000;
    const INTRO_MS = 1500;
...
function drawBumperBands(ctx, state) {
...
function drawPullback(ctx, state, tick) {
    const totalDur = 1200;
