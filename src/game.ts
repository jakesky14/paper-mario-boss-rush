...
export class Game {
  ...
  private startNewTurn(): void {
    ...
    if (this.state.chestPendingRewards) {
      ...
      for (let r = 0; r <= 2; r++) {
        for (let c = 0; c < 12; c++) {
          if (this.state.rings[r].panels[c] === 'empty') candidates.push([r, c]);
        }
      }
      // Shuffle candidates
      ...
      const toPlace: import('./types').PanelType[] = ['plus_one', 'double_power', 'coin', 'coin', 'heal', 'heal'];
      ...
      case 'whole_punch_attempt': {
        ...
        if (state.wholePunchAttemptTimer <= 0) {
          if (state.marioElevated) {
            ...
            this.spawnFlash('player', '#ff0000');
            if (state.playerHp <= 0) { this.transitionTo('game_over'); break; }
